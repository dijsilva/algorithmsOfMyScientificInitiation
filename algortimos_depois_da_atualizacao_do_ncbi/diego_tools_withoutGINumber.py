def blast_out(path_ncbi_info, path_pre_tax):
    ...
    for pos, ptn in enumerate(pre_tax):
        ...
        if len(ncbi_info[pos].split(' ')[0].split('|')) >= 2:
            ...
            if ncbi_info[pos].split(' ')[0].split('|')[0] not in ignore:
                if '[' in ptn:
                    if ']' in ptn:
                        sel_ncbi.append(pos)
    ...
    #nesta etapa é criado um dicionário species com os nomes científicos das espécies E a quantidade de vezes que ela se repete neste sistema.
    #este dado vai ser usado com gabarito depois para ajudar a selecionar as sequências.
    species = {}
    for lists in org:
        for sp in lists:
